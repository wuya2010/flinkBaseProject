...
object FlinkTableApi {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val t  = TableEnv
      ...
      .connect(new FileSystem().path("E:\\01_myselfProject\\Base\\spark_flink_project\\flink_streaming\\src\\main\\resources\\sensor.txt"))
      .withFormat(new Csv())
...
//    //fixme:定义输出结果表
    ...
    //输出表名称： jdbcOutputTable
      ...
      //连接一张表
    val result = TableEnv.from("inputTable")  //获取流式数据


    //表查询 TableApi
    result.select("id,temperature").filter("id='sensor_1'")
   //聚合
    val aggTable = result
      .groupBy("id")
//      .groupBy("key").select("key, value.avg")
    ...
    //sqlc查询2
    val sqlTable2 =  TableEnv.sqlQuery(
      """
