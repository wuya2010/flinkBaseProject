...
//      .reduce((x, y) => SensorReading(x.id, x.timestamp + 1, y.temperature + 10))
      .min("temperature") // fixme: 每来一条数据，输出一次
...
//    aggStream.print("min")
        ...
        // 2. 分流算子测试  fixme: split 这里可以用process 进行数据分流
        ...
        // 3. 合并两条流 ==》 scala 中的 map 对数据直接进行转换和拼接 输出结果 warning> (sensor_2,33.0)
...
//         coMapStream.print("test")  //test> (sensor_2,33.0,high temperature warning)
